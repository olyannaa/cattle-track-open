...
Тулы и ключевой набор инструментов описан в `00-tool-catalog.md`. Сторонние бенчмарки используются только для формирования shortlist. Выбор делается на проверенных CattleTrack-датасетах и end-to-end сценариях приложения.
...
Исследовательский победитель — `Qwen3.5-9B Q4_K_M`; `Qwen3.5-4B Q4_K_M` остается fast/resource fallback. T-lite исключается из model shortlist из-за 0/11 single-write и 0/4 no-tool, несмотря на лучший TTFT.
