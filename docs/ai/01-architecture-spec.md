...
## 10. основной скоуп
Строим и бенчмаркаем ключевой набор инструментов из `00-tool-catalog.md` (6 read + 3 write). Полный каталог реализуется тем же паттерном по мере времени. На защите: полный каталог — как охват продукта, бенчмарк — на выбранном наборе сценариев. Не пытаться довести и измерить все ~55 тулов за 2 недели.
