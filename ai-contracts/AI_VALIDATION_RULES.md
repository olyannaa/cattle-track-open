...
Для MVP tool-calling сейчас нет write-тулов для проверки стельности и отела, поэтому такие команды должны попадать в `no-tool` или будущий backlog, а не в `create_insemination`.
